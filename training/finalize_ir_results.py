...
SEED_RUNS = {
    "seed42": RUNS / "EXP-IR-001-FULL",
    "seed123": RUNS / "EXP-IR-001-SEED123",
    "seed2026": RUNS / "EXP-IR-001-SEED2026",
}

WEIGHTED_RUN = RUNS / "EXP-IR-002-FULL"
...
SEED_SUMMARY_CSV = OUTPUT_DIR / "ir_seed_summary.csv"
SEED_SUMMARY_JSON = OUTPUT_DIR / "ir_seed_summary.json"

PER_LABEL_CSV = OUTPUT_DIR / "ir_per_label_3seed_summary.csv"

ABLATION_CSV = OUTPUT_DIR / "ir_ablation_summary.csv"

FINAL_JSON = OUTPUT_DIR / "ir_final_summary.json"
FINAL_MD = OUTPUT_DIR / "IR_FINAL_SUMMARY.md"
...
def plot_seed42_training():
    ...
    plt.ylabel("BCE loss")
    plt.title(
        "IR 1D-CNN Training and Validation Loss (Seed 42)"
    )
    ...
    plt.title(
        "IR 1D-CNN Validation Metrics (Seed 42)"
    )
...
def plot_per_label_f1(per_label_rows):
    ...
    plt.title(
        "IR Functional-Group Performance Across 3 Seeds"
    )
...
def build_final_report(
    ...
    final_json = {
        ...
        "threshold_calibration_conclusion":
            (
                ...
                "slightly improved validation Macro-F1 but "
                "did not improve overall test Micro-F1. "
        ...
        "class_weighting_conclusion":
            (
                ...
                "recall but reduced precision, Micro-F1, "
                "Macro-F1, and mAP. Ordinary BCE is retained."
            ),
    ...
    lines.append(
        ...
        "可提升 validation Macro-F1，但在独立 scaffold test "
        "上未改善整体 Micro-F1，因此 IR-v1 保留统一阈值 0.5。"
    )
    ...
    lines.append(
        ...
        "模型 Recall 提高，但 Precision、Micro-F1、Macro-F1 "
        "和 mAP 均下降。因此 IR-v1 不采用 class weighting。"
    )
